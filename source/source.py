...
def _play(sound,device=5):
    ...
    with sf.SoundFile(sound) as wf:
        stream = sd.RawOutputStream(samplerate=int(wf.samplerate*speed_cursor),
                                    device=device,
                                    channels=2,
                                    callback=callback,
                                    blocksize=1024,
...
def set_shortcuts():
    ...
    kb.unhook_all()
    kb.add_hotkey('ctrl+plus', partial(inc_dec_slider,0.2))
    kb.add_hotkey('ctrl+ -', partial(inc_dec_slider,-0.2))
    kb.add_hotkey('ctrl+*', reset)
    kb.add_hotkey('ctrl+p', stop_all_sounds)
    for i in list_shortcuts:
        kb.add_hotkey(f'ctrl+{list_shortcuts.index(i)}', partial( read_sound,i))

def slider_event(value):
    global speed_cursor
    speed_cursor=value/100
...
def inc_dec_slider(i):
    global speed_cursor
    speed_cursor+=i
    if speed_cursor<0.1:speed_cursor=0.1
    if speed_cursor>3:speed_cursor=3
    slider.set(speed_cursor*100)
...
#________________________Variables_____________________________________________
...
slider = ct.CTkSlider(root, from_=10, to=300,orientation='vertical', command=slider_event)
slider.grid(row=0,column=3,rowspan = 2)
...
reset_button= ct.CTkButton(root, text='R',width=15,height=15,command=reset)
